...
public enum FindBeeError: Error {
...
public struct FindBee {
    ...
    public func unusedFiles() throws -> [FileInfo] {
        guard !resourceExtensions.isEmpty else {
            throw FindBeeError.noResourceExtension
        }
        guard !searchInFileExtensions.isEmpty else {
            throw FindBeeError.noFileExtension
        }
        
        let allResources = allResourceFiles()
        let usedNames = allUsedStringNames()
        
        return FindBee.filterUnused(from: allResources, used: usedNames).map( FileInfo.init )
    }
